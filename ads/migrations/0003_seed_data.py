...
def seed_data(apps, schema_editor):
    TargetArea = apps.get_model('ads', 'TargetArea')
    TargetAudience = apps.get_model('ads', 'TargetAudience')
...
def reverse_data(apps, schema_editor):
    TargetArea = apps.get_model('ads', 'TargetArea')
    TargetAudience = apps.get_model('ads', 'TargetAudience')
    TargetArea.objects.filter(state="Tamil Nadu", city="Chennai").delete()
    TargetAudience.objects.all().delete()


class Migration(migrations.Migration):
    dependencies = [
        ('ads', '0002_ad_generation_error'),
    ]
